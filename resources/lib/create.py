...
class Main:
    ...
    def _parse_argv(self):
        ...
        group.add_argument(
            "-f", "--source", help="generate file based on either 'settings' or 'tvmaze'")
        parser.add_argument(
            "-i", "--tvmazeids", help="TV Maze IDs (comma sep), 'followed', or 'tags:tagids' (comma sep)")
        parser.add_argument(
            "-l", "--lookback", help="number of days backwards in time to look for episode match")
        parser.add_argument(
            "-s", "--seasons", help="comma separated list of the seasons to create")
        parser.add_argument(
            "-e", "--episodes", help="comma separated list of the number of episodes in each season")
        parser.add_argument(
            "-d", "--dates", help="comma separated list of season dates (or True to use tvmaze airdates)")
        parser.add_argument(
            "-t", "--title", help="title for the Kodi dialog box")
        parser.add_argument(
            "-m", "--msg", help="message used in the Kodi dialog box (or 'tag-based' to use tvmaze tag name based msg)")
        parser.add_argument(
            "-y", "--type", help="the media type for the stub (must be a valid Kodi type)")
        parser.add_argument("-r", "--streamfile", action="store_true",
                            help="output as a stream file instead of a media stub")
    ...
    def _init_vars(self):
        ...
        self.TVMAZE = tvmaze.API(user=config.Get(
            'tvmaze_user'), apikey=config.Get('tvmaze_apikey'))
        self.TAGNAMEMAP = {}
    ...
    def _create_stubs(self):
        ...
        if self.ARGS.source == 'settings':
            ...
            self.LW.log(['invalid argument for source: %s' %
                        self.ARGS.source], 'info')
        else:
    ...
    def _create_stubs_from_args(self, media_type='', ext='disc'):
        ...
        video_path = os.path.join(
            self.DATAROOT, config.Get('tvroot'), video_name)
        try:
            season_list = self.ARGS.seasons.split(',')
            episode_list = self.ARGS.episodes.split(',')
        except AttributeError as e:
            video_path = os.path.join(
                self.DATAROOT, config.Get('movieroot'), video_name)
            season_list = []
            episode_list = []
        ...
        for season in season_list:
            ...
            for e in range(1, max_eps):
                ...
                file_name = '%s.S%sE%s%s.%s' % (
                    video_name, season_num, ep_num, media_type, ext)
                file_path = os.path.join(video_path, file_name)
                self._write_stub(file_path, file_text,
                                 setdate=self.DATELIST[ref])
            ref = ref + 1
        if not season_list:
    ...
    def _create_stubs_from_settings(self, media_type='', ext='disc'):
        ...
        for video in config.Get('videos'):
            ...
            self.LW.log(['checking settings date %s against today %s' %
                        (video.get('date'), today_formatted)], 'info')
            if video.get('date') == today_formatted:
                file_text = self._get_file_text(
                    video.get('name'), title=video.get('title'), msg=video.get('msg'))
                ...
                if video.get('episode'):
                    video_path = os.path.join(
                        self.DATAROOT, config.Get('tvroot'), video_name)
                    file_name = '%s.%s.%s' % (
                        video_name, video.get('episode'), ext)
                else:
                    video_path = os.path.join(
                        self.DATAROOT, config.Get('movieroot'), video_name)
    ...
    def _create_stubs_from_tvmazeids(self, media_type='', ext='disc'):
        ...
        for item in items:
            ...
            success, loglines, show = self.TVMAZE.getShow(
                str(item) + double_embed)
            self.LW.log(loglines)
            if not success:
            ...
            try:
                ...
                self.LW.log(
                    ['no valid show name and/or episode list, skipping'], 'info')
                continue
            ...
            if alternatelists:
                ...
                for alist in alternatelists:
                    if alist.get("network"):
                        ...
                        channel = 'webChannel'
                    try:
                        country_code = alist.get(channel, {}).get(
                            "country", {}).get("code")
    ...
    def _get_tvmaze_ids(self):
        ...
        if self.ARGS.tvmazeids == 'followed':
            ...
            self.LW.log(
                ['continuing with updated list of show ids of:', items], 'info')
        else:
    ...
    def _write_tvmave_stubs(self, ext, file_text, showname, episodes):
        ...
        if self.ARGS.lookback:
            ...
            self.LW.log(
                ['checking for episode matches based on date of %s' % checkdate], 'info')
        for episode in episodes:
            if self.ARGS.lookback:
                self.LW.log(
                    ['have an airdate to check of ' + episode.get('airdate')])
            ...
            if not ep_season and not ep_number:
                self.LW.log(
                    ['need both season and episode number to create a valid file, skipping'], 'info')
                continue
            if ep_name:
            ...
            if self.ARGS.dates:
                self._write_stub(file_path, file_text,
                                 setdate=episode.get('airdate'))
            else:
                self._write_stub(file_path, file_text)
            if config.Get('markacquired'):
                self.LW.log(['marking show as acquired on TV Maze'], 'info')
                time.sleep(self.TVMAZEWAIT)
                success, loglines, results = self.TVMAZE.markEpisode(
                    episode.get('id', 0), marked_as=1)
                self.LW.log(loglines)
    ...
    def _get_file_text(self, name, title='', msg=''):
        ...
        if not msg:
            msg = self.MSG
        if self.ARGS.streamfile:
            the_url = 'plugin://plugin.whereareyou?empty=pad&%s' % urlencode(
                {'title': title, 'message': msg}, quote_via=quote_plus)
            try:
                the_url = the_url + \
                    '&%s' % urlencode({'the_url': self.SHOWURLS[name]})
            except KeyError:
                pass
            return the_url
        else:
