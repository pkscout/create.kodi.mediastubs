...
class Logger(object):
    def __init__(self, logconfig="timed", logformat='%(asctime)-15s %(levelname)-8s %(message)s', logfile='logfile.log',
        ...
        if LOGTYPE == 'file':
            ...
            if logconfig == 'timed':
                lr = logging.handlers.TimedRotatingFileHandler(
                    logfile, when=when, backupCount=numbackups)
            else:
                lr = logging.handlers.RotatingFileHandler(
                    logfile, maxBytes=maxsize, backupCount=numbackups)
    ...
    def _output_file(self, line, loglevel):
        if self.LOGDEBUG or loglevel != self.logger.debug:
            try:
                ...
                self.logger.debug('%s unable to output logline' %
                                  self.LOGPREAMBLE)
    ...
    def _output_xbmc(self, line, loglevel):
        if self.LOGDEBUG or loglevel != xbmc.LOGDEBUG:
            try:
                xbmc.log('%s %s' %
                         (self.LOGPREAMBLE, line.__str__()), loglevel)
            except Exception as e:
                xbmc.log('%s unable to output logline' %
                         self.LOGPREAMBLE, loglevel)
